    ...
    <el-table v-loading="loading" :data="reservationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="220" fixed="right" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-check" @click="handleApprove(scope.row)" v-hasPermi="['space:audit:approve']">通过</el-button>
          <el-button size="mini" type="text" icon="el-icon-close" @click="handleReject(scope.row)" v-hasPermi="['space:audit:reject']">驳回</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listPendingReservation, getReservation, approveReservation, rejectReservation, approveReservationItem, rejectReservationItem } from '@/api/space/reservation'
...
export default {
  ...
  methods: {
    ...
    openAuditDialog(row, action) {
      ...
      this.auditTitle = action === 'approve' ? '通过预约' : '驳回预约'
      this.auditForm.auditOpinion = action === 'approve' ? '审核通过' : ''
      this.auditOpen = true
      this.$nextTick(() => {
    ...
    submitAudit() {
      this.$refs.auditForm.validate(valid => {
        ...
        const request = this.auditAction === 'approve' ? approveReservation : rejectReservation
    ...
    handleApproveItem(row) {
      approveReservationItem(row.itemId, { auditOpinion: '单场次通过' }).then(() => {
        this.$modal.msgSuccess('场次已通过')
        this.handleView(this.detail)
        this.getList()
      })
    },
    handleRejectItem(row) {
      this.$prompt('请输入驳回原因', '驳回场次', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPattern: /\S+/,
        inputErrorMessage: '驳回原因不能为空'
      }).then(({ value }) => {
        return rejectReservationItem(row.itemId, { auditOpinion: value })
      }).then(() => {
        this.$modal.msgSuccess('场次已驳回')
        this.handleView(this.detail)
        this.getList()
      }).catch(() => {})
    },
    handleExport() {
      this.download('space/reservation/export', {
        ...this.queryParams,
        pendingOnly: true
      }, `pending_reservation_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
