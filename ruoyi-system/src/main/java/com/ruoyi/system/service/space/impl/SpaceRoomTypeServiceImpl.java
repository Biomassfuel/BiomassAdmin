...
public class SpaceRoomTypeServiceImpl implements ISpaceRoomTypeService
{
    ...
    {
        validateTypeCodeUnique(spaceRoomType);
        int rows = spaceRoomTypeMapper.updateSpaceRoomType(spaceRoomType);
        if (spaceRoomType.getTypeId() != null && !StringUtils.isBlank(spaceRoomType.getTypeName()))
        {
            spaceRoomMapper.updateRoomTypeByTypeId(spaceRoomType.getTypeId(), spaceRoomType.getTypeName());
        }
        return rows;
    }
    ...
    {
        assertNoRoomsUsingTypes(typeIds);
    ...
    {
        assertNoRoomsUsingTypes(new Long[] { typeId });
    ...
    private void assertNoRoomsUsingTypes(Long[] typeIds)
    {
        if (typeIds == null || typeIds.length == 0)
        {
            return;
        }
        if (spaceRoomMapper.countSpaceRoomByTypeIds(typeIds) > 0)
        {
            throw new ServiceException("该房间类型已被房间使用，不能删除");
        }
    }
}
